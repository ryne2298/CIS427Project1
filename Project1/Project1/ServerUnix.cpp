...
void checkOrCreateUser() {
    ...
    char* sql = "SELECT COUNT(*) FROM Users WHERE user_id = 1;";
    int count = 0;
    sqlite3_stmt* stmt;
    ...
    if (count == 0) {
        char* insert_sql = "INSERT INTO Users (user_id, username, balance) VALUES (1, 'John Doe', 100);";
...
void handleList(int new_s) {
    ...
    // Query Stocks table and send results back to client
    char* sql = "SELECT * FROM Stocks;";
    char response[MAX_LINE] = "200 OK\n";
    rc = sqlite3_exec(db, sql, callback, response, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "SQL error: " << sqlite3_errmsg(db) << std::endl;
        send(new_s, "Database error", 14, 0);
        sqlite3_close(db);
...
void handleBalance(int new_s) {
    ...
    char* sql = "SELECT balance FROM Users WHERE user_id = 1;";
    char response[MAX_LINE] = "200 OK\n";
    rc = sqlite3_exec(db, sql, callback, response, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "SQL error: " << sqlite3_errmsg(db) << std::endl;
